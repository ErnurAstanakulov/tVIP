...
class TransferWorkCell: UITableViewCell {
    ...
    public var pointsButton = UIButton()
    ...
    func fill(model: WorkDocumentsModel) {
        datePeriadLabel.text = model.creation
        transferTypeLabel.text = model.docType?.label
        ownerLabel.text = model.receiver
        sumLabel.text = "\(model.amount ?? 0.0)"
        statusLabel.text = model.state?.code
        pointsButton.tag = model.id
    }
...
extension TransferWorkCell: ViewInitalizationProtocol {
    ...
    func stylizeViews() {
        pointsButton.setImage(AppImage.moreTransfer.uiImage, for: .normal)
        pointsButton.contentVerticalAlignment = .fill
        pointsButton.contentHorizontalAlignment = .fill
    }
}
